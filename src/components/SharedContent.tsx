...
function SharedContent() {
  ...
  useEffect(() => {
    const fetchSharedContent = async () => {
      try {
        const response = await axios.get(`${BASE_URL}/shared/${sharelink}`); //here i made the changes
        ...
        // @ts-ignore
        setError(
          err.response?.data?.message || "Failed to fetch shared content"
        );
      }
    };
